...
class CuadradosMediosApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Algoritmo de Cuadrados Medios")
        self.root.geometry("900x600")
        self.root.configure(bg="#113E79")  # Celeste claro
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            self.root,
            text="Algoritmo de Cuadrados Medios",
            font=("Arial", 16, "bold"),
            bg="#E3F2FD",
            fg="#000203"
        )
        ...
        # Frame para entradas (alineadas a la izquierda)
        ...
        semilla_entry.grid(row=0, column=1, padx=10, pady=5)
        ...
        self.tree.column("Y_i²", width=120, anchor="center")
    ...
    def generar_numeros(self):
        try:
            ...
            if semilla <= 0 or n <= 0:
                raise ValueError("La semilla y las iteraciones deben ser números positivos.")
    ...
    def prueba_medias(self):
        ...
        r_values = []
        for item in items:
            r_str = self.tree.item(item, 'values')[4]  # Columna R_i (índice 4)
        ...
        # Botones
        button_frame = tk.Frame(ventana_prueba, bg="#E3F2FD")
        button_frame.pack(pady=10)
        ...
        def mostrar_histograma(r_values):
            ...
            # Histograma
            n_bins = 10
            counts, bins, patches = ax.hist(r_values, bins=n_bins, alpha=0.7, color='lightblue', edgecolor='darkblue', linewidth=1.5)
        ...
        def exportar_a_txt(r_values, z_value):
            ...
            try:
                with open("prueba_medias.txt", "w", encoding="utf-8") as file:
                    file.write(output)
                messagebox.showinfo("Exportación", "Los resultados se han guardado en 'prueba_medias.txt'")
            except Exception as e:
    ...
    def prueba_varianza(self):
        ...
        r_values = []
        for item in items:
            r_str = self.tree.item(item, 'values')[4]
            r_values.append(float(r_str))
        ...
        ventana_prueba.configure(bg="#E3F2FD")  # Celeste claro

        # Estilo
        style = ttk.Style()
        style.theme_use('clam')
        style.configure("TButton", background="#86888A", foreground="white", font=("Arial", 10))
        style.map("TButton", background=[('active', "#000000")])
        ...
        def exportar_a_txt(r_values, modo, confianza, chi_alpha_2_manual, chi_1_minus_alpha_2_manual):
            ...
            try:
                with open("prueba_varianza.txt", "w", encoding="utf-8") as file:
                    file.write(output)
                messagebox.showinfo("Exportación", "Los resultados se han guardado en 'prueba_varianza.txt'")
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo guardar el archivo: {str(e)}")
    ...
    def prueba_uniformidad(self):
        ...
        style = ttk.Style()
        style.theme_use('clam')
        style.configure("TButton", background="#86898B", foreground="white", font=("Arial", 10))
        style.map("TButton", background=[('active', "#000000")])
        ...
        def exportar_a_txt(r_values, m, confianza):
            ...
            # Guardar en archivo
            try:
                with open("prueba_uniformidad.txt", "w", encoding="utf-8") as file:
                    file.write(content)
                messagebox.showinfo("Exportación", "Los resultados se han guardado en 'prueba_uniformidad.txt'")
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo guardar el archivo: {str(e)}")
