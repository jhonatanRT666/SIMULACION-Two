#!/usr/bin/env python3
"""
Distribución Exponencial (Oscuro-Azulado)
...
class DistribucionExponencial:
    """Distribución Exponencial basada en la inversa de la Gamma"""
    ...
    def generar(self):
        """Genera n valores con la fórmula INV.GAMMA(r; 1; media)"""
        valores = []
        for _ in range(self.n):
            r = random.random()  # número aleatorio entre 0 y 1
            x = gamma.ppf(r, a=1, scale=self.media)
...
class ExponencialApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Distribución Exponencial — Estilo Oscuro-Azulado")
    ...
    def generar(self):
        try:
            ...
            if media <= 0 or n <= 0:
                messagebox.showerror("Error", "La media y n deben ser mayores que 0.")
                return

            dist = DistribucionExponencial(media, n)
    ...
    def mostrar_histograma(self):
        ...
        plt.title("Histograma — Distribución Exponencial", color='#b3e5fc', fontsize=13)
