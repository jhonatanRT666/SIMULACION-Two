...
class DistribucionKerlang:
    """Distribución K-Erlang basada en la inversa de la Gamma"""

    def __init__(self, k, media, n):
        self.k = k
        self.media = media
        self.n = n

    def generar(self):
        """Genera n valores con la fórmula INV.GAMMA(r; k; media/k)"""
        valores = []
        for _ in range(self.n):
            r = random.random()  # número aleatorio entre 0 y 1
            x = gamma.ppf(r, a=self.k, scale=self.media / self.k)
...
class KerlangApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Distribución K-Erlang — Estilo Oscuro-Azulado")
    ...
    def _build_ui(self):
        ...
        tk.Label(frame_top, text="k (forma):", bg=PANEL, fg=FG_TEXT).grid(row=0, column=0, padx=6, pady=4)
        self.entry_k = ttk.Entry(frame_top, width=10)
        self.entry_k.grid(row=0, column=1, padx=6)

        tk.Label(frame_top, text="Media:", bg=PANEL, fg=FG_TEXT).grid(row=0, column=2, padx=6, pady=4)
        self.entry_media = ttk.Entry(frame_top, width=10)
        self.entry_media.grid(row=0, column=3, padx=6)
    ...
    def generar(self):
        try:
            ...
            if k <= 0 or media <= 0 or n <= 0:
                messagebox.showerror("Error", "Todos los valores deben ser mayores que 0.")
                return

            dist = DistribucionKerlang(k, media, n)
    ...
    def mostrar_histograma(self):
        ...
        plt.title("Histograma — Distribución K-Erlang", color='#b3e5fc', fontsize=13)
