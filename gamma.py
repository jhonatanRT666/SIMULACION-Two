...
class DistribucionGamma:
    """Distribución Gamma basada en la fórmula INV.GAMMA(r; media²/varianza; varianza/media)"""
    ...
    def generar(self):
        """Genera n valores con la fórmula indicada"""
        valores = []
        k = (self.media ** 2) / self.varianza       # parámetro de forma (shape)
        theta = self.varianza / self.media          # parámetro de escala (scale)

        for _ in range(self.n):
            r = random.random()  # número aleatorio entre 0 y 1
            x = gamma.ppf(r, a=k, scale=theta)
...
class GammaApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Distribución Gamma — Estilo Oscuro-Azulado")
    ...
    def generar(self):
        try:
            ...
            if media <= 0 or varianza <= 0 or n <= 0:
                messagebox.showerror("Error", "La media, varianza y n deben ser mayores que 0.")
                return

            dist = DistribucionGamma(media, varianza, n)
    ...
    def mostrar_histograma(self):
        ...
        plt.title("Histograma — Distribución Gamma", color='#b3e5fc', fontsize=13)
